...
export function buildIR(blocks, prose = [], programName = null) {
  ...
  // bug: the log stmt no longer emits spdlog::...() — it falls back to
  // std::clog so a `log info foo` works without the user adding
  // `Require: spdlog`. so the dep is no longer auto-added.
...
function parseValue(raw) {
  ...
  // hex (0x..) and binary (0b..) literals. javascript's Number() handles
  // both natively, but the previous version of parseValue only matched
  // the decimal form, so `set hex = 0xFF` slipped through to the
  // bare-token branch and ended up as the string "0xFF" instead of 255.
