...
export function buildIR(blocks, prose = [], programName = null) {
  ...
  for (const b of blocks) {
    if (b.kind === 'require') {
      ...
      const v = parseValue(b.m[0]);
      ir.behaviors.push({ trigger: 'start', body: [{ kind: 'ask', text: v.value, isString: v.isString, name: slug(b.m[0]) }] });
      ...
      if (m) {
        ...
        const stmt = { kind: 'for', iter: m[1], source: v.value, isString: v.isString, body: [] };
...
function parseMainLine(line) {
  ...
  if ((m = line.match(/^(?:set|let)\s+(\w+)\s*=\s*(.+?)\.?$/i))) { const v = parseValue(m[2]); return { kind: 'set', name: m[1], value: v.value, isString: v.isString }; }
  if ((m = line.match(/^call\s+(\w+)\s*(?:\(([^)]*)\))?\.?$/i))) {
    ...
    const args = m[2]
      ...
      : [];
    return { kind: 'call', target: m[1], args };
  }
  ...
  if ((m = line.match(/^for\s+each\s+(\w+)\s+in\s+(.+?)\s*[.:]?\s*$/i))) { const v = parseValue(m[2]); return { kind: 'for', iter: m[1], source: v.value, isString: v.isString }; }
  ...
  if ((m = line.match(/^file_read\s+(\S+)(?:\s+into\s+(\w+))?\.?$/i))) { const v = parseValue(m[1]); return { kind: 'file_read', path: v.value, isString: v.isString, name: m[2] || 'data' }; }
