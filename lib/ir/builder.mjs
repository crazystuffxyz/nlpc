...
export function buildIR(blocks, prose = [], programName = null) {
  ...
  for (const b of blocks) {
    if (b.kind === 'require') {
      ...
      const v = parseValue(b.m[0]);
      // bug: `ask for age` was slugged to `for_age`. use the last word.
      ...
      // bug: lower top-level "GET /foo" with the real method, not a hardcoded verb
      ...
      // bug: `set class = 5` would lower to `auto class = 5;` (reserved). slug appends `_n`.
      // bug: the old code pushed the raw string `"5"` as the value,
      // which the emitter wrapped in cppString - producing
      // `std::string x = "5";` (wrong type, wrong value). use
      // parseValue to coerce numeric and boolean literals to their
      // native JS forms, and tag the IR with the type so the
      // emitter can pick a matching cpp type and not wrap the
      // literal in a c++ string.
      const v = parseValue(b.m[1]);
      const declType = v.isString ? 'string' : (typeof v.value === 'number' ? (Number.isInteger(v.value) ? 'int' : 'double') : (typeof v.value === 'boolean' ? 'bool' : 'string'));
      ir.declarations.push({ kind: 'variable', name: slug(b.m[0]), value: v.value, isString: v.isString, type: declType });
    } else if (b.kind === 'if') {
      // bug: top-level `if X:` captured with body in structured.mjs. lower to a behavior with an `if` stmt.
  ...
  // bug: log uses std::clog now, so spdlog isn't auto-added
  if (usesJson) require('nlohmann-json');
  // bug: http_serve needs cpp-httplib even if the user didn't `Require` it
...
function parseValue(raw) {
  ...
  // hex (0x..) and binary (0b..) literals. decimal-only path stringified them as "0xFF".
  if (/^-?0[xX][0-9a-fA-F]+$/.test(s) || /^-?0[bB][01]+$/.test(s)) {
    const n = Number(s);
    if (Number.isFinite(n)) return { value: n, isString: false };
  }
  if (s === 'true' || s === 'false') return { value: s === 'true', isString: false };
  if ((s.startsWith('"') && s.endsWith('"')) || (s.startsWith("'") && s.endsWith("'"))) {
    // bug: the previous version sliced any string whose first and last
    // chars were quotes - so `set x = "a" + "b"` lowered to a string
    // whose value was `a" + "b`. require the inner content to look
    // like a real c++ string literal: no unescaped quotes, no `+`
    // operator (concatenation is a different construct). if the inner
    // has any of those, fall through to the bare-token path so the
    // emitter routes it through the expression guard.
    if (s.length >= 2) {
      const inner = s.slice(1, -1);
      // reject if the inner contains a literal `+` (concatenation), an
      // unescaped quote, or looks like two quoted strings glued by ` + `
      if (inner.includes('+') || /"/.test(inner) || /'/.test(inner)) {
        // fall through
      } else {
        const unescaped = inner
          ...
          .replace(/\\t/g, '\t')
          .replace(/\x00/g, '\\');
        return { value: unescaped, isString: true };
      }
    }
  }
...
function parseMainLine(line) {
  ...
  if ((m = line.match(/^call\s+(\w+)\s*(?:\(((?:[^()]|\([^()]*\))*)\))?\.?$/i))) {
    // bug: filter empty args and tag each with isString for correct emit routing.
    // bug: the original `[^)]*` for the inner args broke `call add(inc(1), inc(2))`
    // because `[^)]*` stops at the FIRST `)`, which is the closing paren of
    // `inc(1)`, not the outer `call(...)`. allow one level of nested parens
    // inside the args with `(?:\([^()]*\))*` so simple composition works.
    const args = m[2]
  ...
  // bug: inner `for each X in Y:` (inside main) used to capture the trailing `:`. allow [.:]?
  ...
  // bug: file-rename / file-delete stmts were unhandled. `rename FROM to TO`, `delete PATH`.
  // also accept the `file_` prefixed form (`file_rename X to Y`, `file_delete X`) to match
  // the LLM system prompt and the LLM-disambiguated names. accept both at the body level.
  if ((m = line.match(/^(?:file_)?rename\s+(\S+)\s+to\s+(\S+)\.?$/i))) { const a = parseValue(m[1]); const b = parseValue(m[2]); return { kind: 'file_rename', from: a.value, fromIsString: a.isString, to: b.value, toIsString: b.isString }; }
  if ((m = line.match(/^(?:file_)?delete\s+(\S+)\.?$/i))) { const v = parseValue(m[1]); return { kind: 'file_delete', path: v.value, isString: v.isString }; }
  ...
  // raw c++ fallback. restricted to lines that begin with `raw ` so prompt-injection in prose cannot flow through.
