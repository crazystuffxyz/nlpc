...
export function buildIR(blocks, prose = [], programName = null) {
  ...
  for (const b of blocks) {
    if (b.kind === 'require') {
      ...
      // bug: `set class = 5` slug-bumps `class` (reserved). use parseValue
      // to coerce numbers/bools to native js and tag the IR with the type
      // - otherwise emitter would wrap "5" in cppString -> std::string x = "5";.
...
function parseValue(raw) {
  ...
  if ((s.startsWith('"') && s.endsWith('"')) || (s.startsWith("'") && s.endsWith("'"))) {
    // bug: the old version sliced any string whose first+last chars
    // were quotes, so `set x = "a" + "b"` lowered to a string whose
    // value was `a" + "b`. require the inner to look like a real c++
    // string literal: no unescaped quotes, no `+` (that's concat,
    // a different construct). fall through to the bare-token path
    // otherwise.
    if (s.length >= 2) {
