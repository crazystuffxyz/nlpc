...
const PATTERNS = [
  ...
  // bug: http_serve regex didn't allow "serve on port 9090" - accept optional "on"
  ...
  while (i < lines.length) {
    ...
    if (matched) {
      if (matched.kind === 'application') {
        ...
        // bug: `// comment` inside the body used to be passed through to
        // parseMainLine, which returned `unsupported` and the IR validator
        // rejected the whole program. strip comments at gather time.
        const body = [];
        let j = i + 1;
        while (j < lines.length && (/^\s/.test(lines[j]) || lines[j].trim() === '')) {
          const t = lines[j].trim();
          if (t && !t.startsWith('//') && !t.startsWith('#')) body.push(lines[j]);
          j++;
        }
        ...
        if (parenM) {
          // bug: split(' ').pop() kept only the name. parse (type name) pairs and rust-style `name: type`.
          ...
          if (takesM) {
            ...
            const filtered = words.filter(w => !/^(a|an|the|person|user|nothing|none|no|parameters|args|arguments)$/i.test(w));
            if (filtered.length) params = [{ name: filtered[filtered.length - 1], type: null }];
            // `takes nothing`, `takes no parameters`, etc. -> no params.
            // the regex above leaves filtered empty for those, so params
            // stays [] which is what the user means.
          }
        }
        ...
        // nested if/for blocks. strip `//` and `#` comments - the top-level
        // loop strips them on the first pass but not on body gather.
        const body = [];
        let j = i + 1;
        while (j < lines.length && (/^\s/.test(lines[j]) || lines[j].trim() === '')) {
          const t = lines[j].trim();
        ...
        // strip comments at gather time (same fix as main/function).
        const body = [];
        let j = i + 1;
        while (j < lines.length && (/^\s/.test(lines[j]) || lines[j].trim() === '')) {
          const t = lines[j].trim();
