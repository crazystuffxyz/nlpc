...
import { stripRawStmts } from './ir/sanitize.mjs';
...
export async function run({ file, opts, cfg, spinner, runAfter = false }) {
  ...
  if (opts.noLlm || process.env.NLPC_OFFLINE === '1') {
    ...
    try { ir = buildIR(blocks, prose, basename(file, extname(file))); }
      ...
      // bug #14: the LLM can output `raw` stmts containing arbitrary
      // c++. prompt injection in free-form prose can trick the LLM
      // into writing `raw system("...")` to escape the sandbox. strip
      // raw stmts from LLM-produced IR unless --allow-raw was passed.
      if (!opts.allowRaw) {
        const stripped = stripRawStmts(ir);
        if (stripped) console.error(`[nlpc] stripped ${stripped} raw stmt(s) from LLM output (pass --allow-raw to keep)`);
      }
      if (ir.requirements) {
