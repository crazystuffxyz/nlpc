...
export async function vcpkgInstall(workdir, cfg) {
  ...
  // pass --triplet explicitly when configured, so the build doesn't drift to
  // whatever vcpkg auto-detects (msvc by default on this box, which produces
  // .lib files the mingw linker can't read).
  const vcpkgArgs = ['install', '--x-install-root=' + resolve(workdir, 'vcpkg_installed')];
  if (cfg.vcpkgTriplet) vcpkgArgs.push('--triplet=' + cfg.vcpkgTriplet);
  try {
    await run(vcpkgBin, vcpkgArgs, { cwd: workdir, timeout: cfg.timeoutMs });
  } catch (e) {
