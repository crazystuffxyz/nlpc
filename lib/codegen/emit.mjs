...
const EXPR_RE = /^[A-Za-z_0-9][A-Za-z0-9_ .(),+\-*/%<>=!&|^~?{}\[\]'"`]*$/;
function cppExpr(v) {
  if (v === null || v === undefined) return '""';
...
function emitStmt(s, indent, ctx = {}) {
  ...
  switch (s.kind) {
    ...
    case 'http_serve': {
      return `${indent}svr.listen("0.0.0.0", ${isIdent(s.port) ? s.port : '8080'});`;
    }
    case 'http_route': {
      const path = cppString(s.path || '/');
      return `${indent}svr.get(${path}, [](const auto& req, auto& res) { res.set_content("ok", "text/plain"); });`;
    }
    case 'json_load': {
      return `${indent}auto j = nlohmann::json::parse(std::ifstream(${cppString(s.path || 'data.json')}));`;
    }
    case 'json_save': {
      return `${indent}{ std::ofstream _nlpc_out(${cppString(s.path || 'data.json')}); _nlpc_out << nlohmann::json(${cppLiteral(s.value)}).dump(); }`;
    }
    case 'file_read': {
    ...
    case 'file_write': {
      return `${indent}{ std::ofstream _nlpc_out(${cppString(s.path || 'out.txt')}); _nlpc_out << ${cppLiteral(s.value)}; }`;
    }
    case 'log': {
    ...
    case 'for': {
      ...
      childKnown.add(iter);
      const lines = [`${indent}for (auto& ${iter} : ${cppExpr(s.source ?? {})}) {`];
