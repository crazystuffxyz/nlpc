...
export function emitProject(ir, programName) {
  ...
  // bug: openssl used to be added as a forced transitive of
  // cpp-httplib. cpp-httplib's `ssl` feature is opt-in, so adding
  // openssl by default pulled in a heavy ssl build even for plain
  // HTTP. only add it when the user explicitly required it.
  // (No automatic transitive adds at this time; see the
  // `Require the openssl library.` path if you need SSL.)
