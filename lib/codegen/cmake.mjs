...
function renderCmake(name, deps, ir) {
  ...
  const findMap = {
    ...
    // spdlog's CMake config does find_dependency(fmt) itself, but only
    // when SPDLOG_FMT_EXTERNAL is set. emit the find_package(fmt) call
    // ourselves so vcpkg's spdlog with the fmt feature resolves cleanly.
    'spdlog': ['set(SPDLOG_FMT_EXTERNAL ON)', 'find_package(spdlog CONFIG REQUIRED)'],
    'spdlog-header-only': ['set(SPDLOG_FMT_EXTERNAL ON)', 'find_package(spdlog CONFIG REQUIRED)'],
  ...
  for (const d of deps) {
    ...
    else if (d === 'spdlog') links.push('fmt::fmt', 'Threads::Threads');
